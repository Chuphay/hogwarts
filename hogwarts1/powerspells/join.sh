...
do
	...
	# read -sp 'Password: ' passvar
        useradd -m -d $location/castle/gryffindor_tower/dorms/$uservar \
             -s /bin/bash -e `date -d "60 days" +"%Y-%m-%d"` $uservar
        ...
        echo "cd $location/castle/gryffindor_tower" >> $location/castle/gryffindor_tower/dorms/$uservar/.bashrc
        echo "source /usr/local/bin/hogwarts/bash_error.sh" >> $location/castle/gryffindor_tower/dorms/$uservar/.bashrc
        echo 'umask 002' >> $location/castle/gryffindor_tower/dorms/$uservar/.profile
        echo 'Welcome' >> $location/castle/gryffindor_tower/dorms/$uservar/.profile
        usermod -a -G year_one $uservar
        # usermod -g year_one $uservar
        # usermod -a -G $uservar $uservar
        echo "year_one 1" > /etc/hogwarts/$uservar
        echo -e "You are all setup! Please re-log into hogwarts by using the following command: \e[96mssh $uservar@hogwarts.tech\e[0m"
        break
    fi
done
